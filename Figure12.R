...
for (i in 1:8){
  ...
  bike <- location |>  
    pivot_longer(traffictype) |> filter(name == "F__of_Bicycles") |>
    dplyr::select(value, datetime)
  
  full_date_df <- data.frame(
    datetime = seq.POSIXt(min(bike$datetime), max(bike$datetime), by = "hour")
  )
  newbike <- merge(full_date_df, bike, by = "datetime", all.x = TRUE)
  newbike <- newbike[!duplicated(newbike$datetime),]
  newbike$value <- na.approx(newbike$value)
  
  z <- msts(newbike$value, seasonal.periods = c(7*24), 
            start = 1) 
...
p <- ggplot(qnew,aes(x=Time,y=value,col=Location)) + geom_line(size=1) + 
  ...
  ggtitle("Trend Component of STL Decomposition for Bicycle Traffic (Averaged)") + 
