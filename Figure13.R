...
for (i in 1:8){
  ...
  road <- location |>  
    pivot_longer(traffictype) |> filter(name == "F__of_Road_Vehicles") |>
    dplyr::select(value, datetime)
  
  full_date_df <- data.frame(
    datetime = seq.POSIXt(min(road$datetime), max(road$datetime), by = "hour")
  )
  newroad <- merge(full_date_df, road, by = "datetime", all.x = TRUE)
  newroad <- newroad[!duplicated(newroad$datetime),]
  newroad$value <- na.approx(newroad$value)
  
  z <- msts(newroad$value, seasonal.periods = c(7*24), 
            start = 1) 
...
p <- ggplot(qnew,aes(x=Time,y=value,col=Location)) + geom_line(size=1) + 
  ...
  ggtitle("Trend Component of STL Decomposition for Road Traffic (Averaged)") + 
